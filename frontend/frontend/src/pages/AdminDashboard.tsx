    ...
    Promise.all([
      ...
      .then(([b, s, sc, a]) =>
        setData({
          barbers: b.length,
          services: s.length,
          schedules: sc.filter((x) => !x.appointment).length,
        ...
        <>
          ...
          <div className="mt-8 rounded-3xl border border-amber-500/20 bg-amber-500/5 p-6">
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-400">
              Cadastre barbeiros e serviços, mantenha os horários disponíveis no
              banco e acompanhe os agendamentos por aqui. O backend atual não
              possui criação de horários, então eles precisam ser inseridos no
              banco enquanto essa rota não existir.
            </p>
          </div>
        </>
      )}
