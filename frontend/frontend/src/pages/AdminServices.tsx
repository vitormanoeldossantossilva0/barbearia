...
export function AdminServices() {
  ...
  const openService = (service?: Service, forcedTopicId?: number) => {
    ...
    setEditing(
      service ?? {
        ...
        price: 0,
        category: "CORTE",
        topicId: typeof nextTopic === "number" ? nextTopic : null,
      },
    );
  ...
  return (
    <AdminLayout>
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-zinc-500">
            Crie os tópicos que aparecerão na Home e coloque cada serviço dentro
            do tópico certo. Os preços continuam exclusivos de cada barbeiro.
          </p>
        </div>
        {canManageTopics && (
          <button
            type="button"
            onClick={() => openTopic()}
            className="rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-black text-zinc-950 hover:bg-amber-400"
          >
            + Novo tópico
          </button>
        )}
        ...
        <div className="space-y-8">
          <section>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              {topics.map((topic) => {
                const topicServices = normalServices.filter(
                  (service) => service.topicId === topic.id,
                );
                const topicCombos = combos.filter(
                  (service) => service.topicId === topic.id,
                );
                return (
                  <section
                  ...
                  >
                    <div className="relative mb-4 h-28 overflow-hidden rounded-xl border border-white/10">
                      <img
                        src={topic.imageUrl || "/images/barbershop-bg.png"}
                        alt=""
                      ...
                      {canManageTopics && (
                        <div className="flex gap-1">
                          <button
                            ...
                            className="rounded-lg px-2.5 py-2 text-xs font-bold text-zinc-400 hover:bg-white/5 hover:text-white"
                          >
                            Editar
                          </button>
                          <button
                            ...
                            className="rounded-lg px-2.5 py-2 text-xs font-bold text-red-300 hover:bg-red-500/10"
                          >
                            Excluir
                          </button>
                        </div>
                    ...
                    <button
                      type="button"
                      onClick={() => openService(undefined, topic.id)}
                      className="mt-4 w-full rounded-xl border border-dashed border-amber-500/30 bg-amber-500/5 py-2.5 text-sm font-black text-amber-400 hover:bg-amber-500/10"
                    >
                      + Adicionar serviço neste tópico
                    </button>
                    <div className="mt-4 grid gap-3">
                      ...
                      {topicServices.length === 0 &&
                        topicCombos.length === 0 && (
                          <div className="rounded-xl border border-dashed border-white/10 p-4 text-sm text-zinc-600">
                            Nenhum serviço neste tópico.
                          </div>
                        )}
                    </div>
                  </section>
          ...
          {items.some((service) => !service.topicId) && (
            <section className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
              ...
              <p className="mt-1 text-sm text-zinc-400">
                Edite estes serviços e escolha em qual tópico eles devem
                aparecer na Home.
              </p>
      ...
      {editing && (
        <Modal
          title={editing.id ? `Editar ${editing.name}` : "Novo serviço"}
        ...
        >
          <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[.02] p-4">
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {normalServices.map((service) => (
                    <label
                      key={service.id}
                      className="flex cursor-pointer items-center gap-3 rounded-xl bg-zinc-950 p-3 text-sm text-zinc-300"
                    >
                      <input
                        type="checkbox"
            ...
            <button
              ...
              disabled={saving}
              onClick={saveService}
              className="w-full rounded-xl bg-amber-500 py-3 font-black text-zinc-950 hover:bg-amber-400 disabled:opacity-50"
            >
      ...
      {topicModal && (
        <Modal
          title={editingTopic ? `Editar ${editingTopic.name}` : "Novo tópico"}
          onClose={() => !savingTopic && setTopicModal(false)}
        >
          <div className="space-y-4">
            <label className="block text-sm font-bold">
              Nome do tópico
            ...
            <label className="block text-sm font-bold">
              ...
              <textarea
                ...
                rows={3}
                placeholder="Uma breve descrição que aparecerá na Home."
                className="mt-2 w-full rounded-xl border border-white/10 bg-zinc-950 px-4 py-3 font-normal outline-none focus:border-amber-500"
              />
            </label>
            <div>
              ...
              <ImageUploadButton
                ...
                onChange={(value) =>
                  setTopicForm((current) => ({ ...current, imageUrl: value }))
                }
                label="Escolher imagem do tópico"
              ...
              {topicForm.imageUrl && (
                <button
                  type="button"
                  onClick={() =>
                    setTopicForm((current) => ({ ...current, imageUrl: "" }))
                  }
                  className="mt-2 text-xs font-bold text-zinc-500 hover:text-red-300"
                >
                  Remover imagem
                </button>
              )}
            ...
            <button
              ...
              className="w-full rounded-xl bg-amber-500 py-3 font-black text-zinc-950 hover:bg-amber-400 disabled:opacity-50"
            >
              {savingTopic ? "Salvando..." : "Salvar tópico"}
            </button>
          </div>
        </Modal>
      )}
      ...
      {deleting && (
        <Modal title="Excluir serviço" onClose={() => setDeleting(null)}>
          <p className="text-sm leading-6 text-zinc-400">
            ...
            <strong className="text-white">{deleting.name}</strong>? Essa ação
          ...
          <div className="mt-5 flex gap-3">
            <button
              ...
              className="flex-1 rounded-xl border border-white/10 py-3 font-bold text-zinc-300 hover:bg-white/5"
            >
              Cancelar
            </button>
            <button
              ...
              className="flex-1 rounded-xl bg-red-500 py-3 font-black text-white hover:bg-red-400"
            >
              Excluir
            </button>
          </div>
