...
const categoryMeta: Record<Exclude<ServiceCategory, "COMBO">, { title: string; description: string }> = {
  CORTE: { title: "Cortes", description: "Cabelo, degradê, social, freestyle e outros estilos." },
  BARBA: { title: "Barba", description: "Serviços de barba com preço próprio." },
  SOBRANCELHA: { title: "Sobrancelha", description: "Serviços de sobrancelha." },
  PINTURA: { title: "Pinturas", description: "Pintura, platinado, luzes e outras técnicas." },
};
...
export function AdminServices() {
  ...
  const open = (service?: Service, forcedCategory?: ServiceCategory) => {
    ...
    setEditing(service ?? { id: 0, name: "", price: 0, category: nextCategory });
    ...
    <AdminLayout>
      <div className="mb-6">
        ...
        <p className="mt-1 max-w-2xl text-sm leading-6 text-zinc-500">
          Organize seus serviços por categoria e monte combos sem misturar opções redundantes. Cada preço continua sendo exclusivo do barbeiro logado.
        </p>
      </div>

      {error && (
        <div role="alert" className="mb-4 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-300">
          {error}
        </div>
      )}
      {message && (
        ...
        <div className="space-y-8">
          <section>
            <div className="mb-4 flex items-end justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[.2em] text-amber-500">Serviços individuais</p>
                <h3 className="mt-1 text-xl font-black">Cortes e cuidados</h3>
                <p className="mt-1 text-sm text-zinc-500">Cabelo, barba e sobrancelha ficam separados para facilitar a escolha do cliente.</p>
              </div>
            </div>
            <div className="space-y-6">
              {normalCategories.slice(0, 3).map((itemCategory) => {
                const categoryItems = normalServices.filter((service) => service.category === itemCategory);
                return (
                  <div key={itemCategory}>
                    <div className="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <h4 className="font-black">{categoryMeta[itemCategory].title}</h4>
                        <p className="text-xs text-zinc-600">{categoryMeta[itemCategory].description}</p>
                      </div>
                      <button type="button" onClick={() => open(undefined, itemCategory)} className="rounded-lg bg-amber-500 px-3 py-2 text-xs font-black text-zinc-950">
                        + Novo
                      </button>
                    </div>
          ...
          <section>
            <div className="mb-4 flex items-end justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[.2em] text-amber-500">Técnicas</p>
                <h3 className="mt-1 text-xl font-black">Pinturas</h3>
                <p className="mt-1 text-sm text-zinc-500">Platinado, luzes, pintura e outras técnicas ficam em uma área própria.</p>
              </div>
              <button type="button" onClick={() => open(undefined, "PINTURA")} className="rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-black text-zinc-950">
                + Nova pintura
              </button>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {normalServices.filter((service) => service.category === "PINTURA").map(serviceCard)}
              {normalServices.filter((service) => service.category === "PINTURA").length === 0 && (
                <div className="rounded-2xl border border-dashed border-white/10 bg-white/[.02] p-5 text-sm text-zinc-600">
          ...
          <section>
            <div className="mb-4 flex items-end justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[.2em] text-amber-500">Combinações</p>
                <h3 className="mt-1 text-xl font-black">Combos</h3>
                <p className="mt-1 text-sm text-zinc-500">Monte uma oferta com serviços diferentes e defina um único preço para o combo.</p>
              </div>
              <button type="button" onClick={() => open(undefined, "COMBO")} className="rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-black text-zinc-950">
                + Novo combo
              </button>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="space-y-4">
            {category === "COMBO" && (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs leading-5 text-zinc-400">
                Escolha os serviços que formarão o combo. Você pode combinar categorias diferentes, mas não pode repetir a mesma categoria.
              </div>
            )}
            ...
            <label className="block text-sm font-bold">
              Nome
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder={category === "COMBO" ? "Ex.: Corte + Barba" : "Ex.: Corte degradê"} className="mt-2 w-full rounded-xl border border-white/10 bg-zinc-950 px-4 py-3 font-normal text-white outline-none focus:border-amber-500" />
            </label>
            <label className="block text-sm font-bold">
              Preço
              <input value={price} onChange={(e) => setPrice(e.target.value)} inputMode="decimal" placeholder="30,00" className="mt-2 w-full rounded-xl border border-white/10 bg-zinc-950 px-4 py-3 font-normal text-white outline-none focus:border-amber-500" />
            </label>

            {category === "COMBO" && (
              <div>
                <div className="mb-2">
                  <p className="text-sm font-bold">Quais serviços fazem parte deste combo?</p>
                  <p className="mt-1 text-xs leading-5 text-zinc-500">Escolha no mínimo dois. O sistema impede colocar duas opções da mesma categoria.</p>
                </div>
                <div className="max-h-64 space-y-2 overflow-y-auto rounded-xl border border-white/10 bg-zinc-950 p-3">
                    ...
                    return (
                      ...
                      >
                        <span>
                          ...
                          <span className="text-xs text-zinc-600">{categoryMeta[service.category].title}</span>
                        </span>
                  ...
                  {normalServices.length === 0 && <p className="p-3 text-sm text-zinc-600">Cadastre serviços individuais antes de criar um combo.</p>}
                </div>
              </div>
            )}
