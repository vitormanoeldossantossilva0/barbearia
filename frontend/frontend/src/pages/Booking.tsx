...
function PublicFooter() {
  return (
    <footer className="border-t border-white/10 bg-transparent px-4 py-8 text-center text-sm text-zinc-400">
...
export function Booking() {
  ...
  useEffect(() => {
    ...
    setError("");
    serviceService
      .list(barberId, shopSlug || undefined)
      .then(setServices)
      .catch((e) => setError(e.message))
  ...
  const handlePhoneChange = (value: string) => {
    setCustomerPhone(formatPhone(value));
  };
  ...
  return (
    <div className="relative isolate min-h-screen overflow-hidden bg-zinc-950 text-white">
      ...
      <PublicHeader shopSlug={shopSlug || undefined} />
      <main className="relative isolate min-h-screen overflow-hidden px-4 py-12 sm:px-6">
        <div className="mx-auto max-w-4xl">
              ...
              {steps.map((label, i) => (
                <div key={label} className="flex  flex-1 items-center">
                  <div
                    ...
                    className={`mx-auto grid h-10 w-10 shrink-0 place-items-center rounded-full text-3 font-black shadow-lg ${i <= step ? "bg-amber-500 text-zinc-950" : "bg-zinc-900/95 text-zinc-400 ring-1 ring-white/10"}`}
                  >
                    {i + 1}
                  </div>
                  {i < steps.length - 1 && (
                    <span
                      aria-hidden="true"
                      className={`ml-1 h-px flex-1 ${i < step ? "bg-amber-500" : "bg-white/15"}`}
                    />
                  )}
                </div>
              ))}
            </div>
            <div className="mt-5 hidden grid-cols-5 gap-1.5 sm:grid sm:gap-2">
              {steps.map((label, i) => (
                <div
                  key={label}
                  className={`flex min-h-14 flex-col items-center justify-center rounded-xl px-1 py-2 text-center text-[10px] font-bold leading-tight sm:min-h-0 sm:flex-row sm:gap-1 sm:rounded-full sm:px-2 sm:text-xs ${i <= step ? "bg-amber-500 text-zinc-950" : "bg-zinc-900 text-zinc-300"}`}
                >
                  <span className="text-xs sm:text-inherit">{i + 1}.</span>
                  <span>{label}</span>
                </div>
              ))}
            </div>
          ...
          <section className="rounded-3xl border border-white/10 bg-zinc-950/90 p-5 shadow-2xl backdrop-blur-sm sm:p-8">
            <div className="mb-6 rounded-2xl border border-white/10 bg-zinc-900/95 px-5 py-4 sm:hidden">
              <p className="text-xs font-black uppercase tracking-[.18em] text-amber-500">
                {steps[step]}
              </p>
            </div>
                            ...
                            .map((service) => {
                              ...
                              return (
                                <button 
                                  ...
                                  onClick={() => toggleService(service)}
                                  className={`hover:cursor-pointer flex items-start justify-between gap-4 rounded-2xl border p-4 text-left transition  ${
                                    selected
                            ...
                            className={`rounded-xl border px-4 py-3 text-sm font-bold ${
                              unavailable
                                ...
                                : selected
                                  ...
                                  : "hover:cursor-pointer border-white/30 bg-zinc-950 text-zinc-300 hover:border-amber-500/50"
                            }`}
                          >
                            {unavailable ? s.time : s.time}
                          </button>
                        );
                      })}
                    </div>
