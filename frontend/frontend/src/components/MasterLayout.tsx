...
export function MasterLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    authService
      .me()
      .then(({ user }) => {
        if (user.role !== "MASTER") throw new Error();
        setEmail(user.email);
      })
      .catch(() => {
        authService.logout();
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-white/10 bg-zinc-900/70 p-5 lg:block">
        <Logo />
        <div className="mt-7 rounded-xl border border-amber-500/20 bg-amber-500/5 p-3">
        ...
        <nav className="mt-3 space-y-1">
          {links.map(([to, label]) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `block rounded-xl px-3 py-3 text-sm font-medium ${isActive ? "bg-amber-500 font-bold text-zinc-950" : "text-zinc-400 hover:bg-white/5 hover:text-white"}`
              }
            >
              {label}
            </NavLink>
      ...
      <main className="min-h-screen lg:pl-64">
        <header className="sticky top-0 z-30 border-b border-white/10 bg-zinc-950/90 px-4 py-4 backdrop-blur sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-zinc-500">Painel master</p>
          ...
          <nav className="mt-4 flex gap-2 overflow-x-auto lg:hidden">
            {links.map(([to, label]) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `whitespace-nowrap rounded-full px-3 py-1.5 text-xs ${isActive ? "bg-amber-500 font-bold text-zinc-950" : "bg-white/5 text-zinc-400"}`
                }
              >
                {label}
              </NavLink>
